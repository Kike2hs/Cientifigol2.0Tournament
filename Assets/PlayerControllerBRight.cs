...
public class PlayerControllerBRight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        { // MOVERSE A LA IZQUIERDA, SIGUIENDO AL BALON
        ...
        {
            rigidBody2D.velocity = new Vector2(moveSpeed, rigidBody2D.velocity.y); // MOVERSE A LA DERECHA, SIGUIENDO AL BALON
        }
    ...
    // Este método puede ser llamado desde el PlayerControllerA para iniciar la habilidad especial
    public void ActivateSpecialAbilityFromPlayerA(float heightMultiplier)
    {
        StartSpecialAbility(heightMultiplier);
    }

    // Este método puede ser llamado desde el PlayerControllerA para detener la habilidad especial
    public void DeactivateSpecialAbilityFromPlayerA(float heightMultiplier)
    {
        EndSpecialAbility(heightMultiplier);
    }

    // Este método puede ser llamado desde el PlayerControllerA para iniciar el salto
    public void ActivateJumpFromPlayerA()
    {
        Jump();
    }

    // Este método puede ser llamado desde el PlayerControllerA para detener el salto
    public void DeactivateJumpFromPlayerA()
    {
        ReleaseJump();
    }
}
